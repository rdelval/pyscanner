...
def scan(ip, filename):
        ...
        if r.status_code == 200:
                ...
                print("Saved pdf to {0}".format(filename))
        else:
...
if __name__ == "__main__":
        ...
        filename = arguments['<filename>'] if arguments['<filename>'] else 'output.pdf'
        stat = status(ip)
        if stat == "Idle":
                exit_code = scan(ip, filename)
        else:
